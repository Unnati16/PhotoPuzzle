...
public class PuzzleBoardView extends View {
    ...
    public void shuffle() {
        if (animation == null && puzzleBoard != null) {
            for(int i=1;i<=30;i++)
            {
                ...
                int min_index=0;
                int randm_index=min_index+ (int)(Math.random()*((max_index-min_index)+1));
                PuzzleBoard shuffled_board=puzzleBoard.neighbours().get(randm_index);
                puzzleBoard=shuffled_board;
                invalidate();//update/refresh UI
            }
            // Do something.
        }
    }
    ...
    public void solve() {
        PriorityQueue<PuzzleBoard> boardQueue =new PriorityQueue<PuzzleBoard>(11, new Comparator<PuzzleBoard>() {
            @Override
        ...
        while(!boardQueue.isEmpty())
        {
            ...
            Log.e("unnati","Executed with"+board_lowest_priority.steps);
            if(!board_lowest_priority.resolved())
            {
                for(int i=0;i<board_lowest_priority.neighbours().size();i++)
                {
                    if(board_lowest_priority.getPreviousBoard()!=null&&!board_lowest_priority.getPreviousBoard().equals(board_lowest_priority.neighbours().get(i))) {
